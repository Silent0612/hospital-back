...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE TABLE_NAME = 'medicalrecord' AND COLUMN_NAME = 'record_code'
    ) THEN
        ALTER TABLE medicalrecord ADD COLUMN record_code VARCHAR(50) COMMENT '病历记录编码';
    END IF;
    
    -- 添加 state 字段
    IF NOT EXISTS (
        SELECT * FROM INFORMATION_SCHEMA.COLUMNS 
        WHERE TABLE_NAME = 'medicalrecord' AND COLUMN_NAME = 'state'
    ) THEN
        ALTER TABLE medicalrecord ADD COLUMN state INT DEFAULT 0 COMMENT '状态：0-待就诊，1-已就诊，2-已取消';
    END IF;
END //
DELIMITER ;
...
CREATE TABLE `appointment` (
  ...
  `visit_type` int DEFAULT NULL COMMENT '就诊类型',
  `description` varchar(500) DEFAULT NULL COMMENT '症状描述',
  `fee` decimal(10,2) DEFAULT NULL COMMENT '费用',
  `status` int NOT NULL DEFAULT '0' COMMENT '状态：0待就诊，1已完成，2已取消',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `diagnose_result` text COMMENT '诊断结果',
  `prescription` text COMMENT '处方信息',
  `user_name` varchar(50) DEFAULT NULL COMMENT '患者姓名',
  `user_gender` int DEFAULT NULL COMMENT '患者性别：0女，1男',
  `user_age` int DEFAULT NULL COMMENT '患者年龄',
